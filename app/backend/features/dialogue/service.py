...
# Uses pushback_dialogue.md system prompt
...
async def generate_dialogue_turn(
    ...
    character = db.query(CharacterTemplate).filter(
        CharacterTemplate.name == scene.character_id
    ).first()
    if not character:
        character_personality = f"{scene.character_id} (economics character)"
    else:
    ...
    # Calculate remaining prompts (max 3-4 exchanges per scene)
    student_turns = len([t for t in conversation_history if t.role == "student"])
    max_prompts_remaining = max(0, 3 - student_turns)

    # Build runtime context matching pushback_dialogue.md expected input
    runtime_context = {
        "character": {
        ...
        "student_response": {
            "type": "freeform",  # Could detect multi_choice vs freeform
