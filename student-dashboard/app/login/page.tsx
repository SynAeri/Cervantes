// Student login page - simple student ID entry
// Replicates old login UI style exactly
...
export default function LoginPage() {
  const { loginWithStudentId, loading: authLoading } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await loginWithStudentId(studentId.trim());

      if (!result.success) {
        setError(result.message || 'Student ID not found');
      }
    } catch (err) {
